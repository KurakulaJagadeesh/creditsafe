...
const importSchema = (payload) => payload.criteriaSets.map((item) => {
	...
	if (item.address) {
		if (item.address.postCode) {
			tempItem.address.postCode = item.address.postCode;
			delete tempItem.address.postCode;
		}
	}
	return item;
});
...
const outputIntersection = (key, value, reference) => {
	...
	if (key === 'allowedValues') {
		...
		if (allowedValues && referenceAllowedValues) {
			// deepcode ignore ArrayMethodOnNonArray: <imported code from Connect. Lack sufficient domain knowledge to address properly>
...
export const mergeCriterias = (countryCodes, criteriaObject) => {
	...
	const inputObj = {
		...
		countriesCriteria: Object.entries(criteriaObject).filter(([key]) => countryCodes.includes(key)).map(([_, value]) => value),
	};
