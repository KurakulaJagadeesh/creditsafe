...
export const generateMappedQueryTerms = (inputObj, mappingFunc, preprocessingFunc = undefined) => {
	const {
		activityCodeDescription, countryCode, exact, groupResults, includePreviousName, increaseRecall, isGGS, pageSize, page, searchMode, userId, ...inputObjectExCountryCode
	} = inputObj;
	...
	const relevantApiParams = omit(
		...
		apiRequestFilterFields[countryCode] || apiRequestFilterFields.default,
...
export const runCompanyNameMatchPhraseVariationsSearch = (inputObj) => {
	...
	const preprocessingFunc = (mappedKeys, value) => {
		const companyName = Object.hasOwn(inputObj, 'name') ? inputObj.name.split(/\s/) : undefined;
...
export const runFallbackMatch = (inputObj) => {
	...
	const mappingFunc = ((mappedKeys, values) => {
		const queries = values.map((value) => ({
			multi_match: {
				...multiMatchTemplate,
				operator: mappedKeys.some((key) => key === 'name') ? 'OR' : 'AND',
				query: value,
...
export const textFieldFallbackQuery = { generateQueryPartial: runFallbackMatch, matchScore: 50 };
